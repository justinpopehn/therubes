...
# Verified Historical Baseline (2006 - 2025)
all_time_stats = {
    "Justin Popehn":    {"wins": 144, "losses": 116, "pf": 26291.4},
    "Nick Wittrock":    {"wins": 142, "losses": 118, "pf": 26691.0},
    "Grant Salzl":      {"wins": 139, "losses": 121, "pf": 25512.0},
    "Jake Childers":    {"wins": 134, "losses": 126, "pf": 26336.2},
    "Scott Schroeder":  {"wins": 132, "losses": 128, "pf": 26240.0},
    "Adam Backes":      {"wins": 122, "losses": 125, "pf": 24758.2},
    "Jay Kiess":        {"wins": 120, "losses": 140, "pf": 25267.4},
    "Bill Loesch":      {"wins": 111, "losses": 123, "pf": 23345.1},
    "Dustin Schlangen": {"wins": 111, "losses": 149, "pf": 24563.4},
    "Adam Willard":     {"wins": 104, "losses": 117, "pf": 21791.0},
    "Cole Schmitz":     {"wins": 19,  "losses": 7,   "pf": 2448.0},
    "Steve Benda":      {"wins": 9,   "losses": 17,  "pf": 2354.0}
}
...
try:
    ...
    for team in league.teams:
        ...
        if owner not in all_time_stats:
            all_time_stats[owner] = {"wins": 0, "losses": 0, "pf": 0.0}
...
rank = 1
for owner, stats in sorted(all_time_stats.items(), key=lambda x: (x[1]['wins'], x[1]['pf']), reverse=True):
    total = stats['wins'] + stats['losses']
    pct = round((stats['wins'] / total * 100), 1) if total > 0 else 0
    win_pct_str = f"{pct}%"
    html_output += f"    <tr><td>{rank}</td><td>{owner}</td><td>{stats['wins']}</td><td>{stats['losses']}</td><td>{win_pct_str}</td><td>{round(stats['pf'], 2)}</td></tr>\n"
    rank += 1
...
print("SUCCESS! Generated updated leaderboard.html")
